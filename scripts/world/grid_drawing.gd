		...
		match cell.type:
			...
			GridWorld.CellType.ORE:
				...
				if ore_texture != null:
					...
					grid.draw_rect(rect, _dirt_color(cell.layer), true)
				# Ore nugget — dull gray-brown once the vein is depleted.
				var inner: Rect2 = rect.grow(-8)
				var nugget_color: Color = Color(0.45, 0.38, 0.3) if grid._mining.is_depleted(cell) else GameManager.COLOR_RUST
				grid.draw_rect(inner, nugget_color, true)
			...
			GridWorld.CellType.LAVA:
				# Molten flood: dark red body with a brighter roiling core.
				var lava_wave: float = 0.5 + 0.5 * sin(Time.get_ticks_msec() / 200.0 + float(hash(pos) % 100))
				grid.draw_rect(rect, Color(0.7, 0.14, 0.02), true)
				grid.draw_rect(rect.grow(-4.0 - 3.0 * lava_wave), Color(1.0, 0.42 + 0.25 * lava_wave, 0.08), true)
			GridWorld.CellType.MAGMA_ROCK:
				grid.draw_texture_rect(grid._MAGMA_ROCK_TEXTURE, rect, false)
				# Cooling ember flicker so the rock reads as fresh from the lava.
				var ember_wave: float = 0.5 + 0.5 * sin(Time.get_ticks_msec() / 300.0 + float(hash(pos) % 100))
				grid.draw_rect(rect, Color(1.0, 0.4, 0.1, 0.04 + 0.06 * ember_wave), true)
			GridWorld.CellType.FRESH_ORE:
				grid.draw_texture_rect(grid._FRESH_ORE_TEXTURE, rect, false)
			GridWorld.CellType.SOLID_ROCK:
				# Cave-in rubble: dark boulders until the rock settles.
				grid.draw_rect(rect, Color(0.22, 0.21, 0.24), true)
				_draw_boulders(rect)
				grid.draw_rect(rect, GameManager.COLOR_SHADOW, false, 1.0)
...
## Cave-in rubble: a few darker boulder lumps over the flat rock fill
## (seeded per cell so the shapes are stable frame to frame).
func _draw_boulders(rect: Rect2) -> void:
	var rng := RandomNumberGenerator.new()
	rng.seed = hash(rect.get_center())
	for i in range(3):
		var offset: Vector2 = Vector2(rng.randf_range(-7.0, 7.0), rng.randf_range(-6.0, 6.0))
		var radius: float = rng.randf_range(5.0, 9.0)
		grid.draw_circle(rect.get_center() + offset, radius, Color(0.32, 0.31, 0.35))
		grid.draw_arc(rect.get_center() + offset, radius, 0, TAU, 10, Color(0.14, 0.13, 0.16), 1.0)
