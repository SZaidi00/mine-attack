...
- `settings_manager.gd`
  ...
  - Because of the stretch setup (`canvas_items`/`expand` + `stretch/scale`), any 16:9 window size keeps the same logical 1920×1080 layout — switching resolution only changes render sharpness.
...
- **Viewport is 2560×1440** (`window/size` in `project.godot`, stretch `canvas_items`/`expand` with `window/stretch/scale=1.333333`). The stretch scale renders the 1920×1080-designed UI and world 4/3 larger at 1440p, so icons, text, and layout keep their original proportions without HUD code changes (`get_viewport().get_visible_rect()` still reports the logical 1920×1080). Camera zoom range is 0.65–2.0 (default 1.0); HUD layout is anchored. The Web export injects full-bleed canvas CSS via `html/head_include`.
