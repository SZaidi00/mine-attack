...
- `player_controller.gd`
  ...
  - Provides UI callbacks: `train_unit(unit_id)`, `upgrade_miner()`, `upgrade_fighter(unit_id)`, `set_stance(stance)`, `set_view(underground)`, `kill_selected()`.
...
- `ai_controller.gd`
  - Tick-driven AI with separate timers for economy (`ENEMY_DECISION_INTERVAL` = 2s, scaled by the difficulty `upgrade_speed`), mining (1s), attack waves (`ENEMY_ATTACK_WAVE_INTERVAL` = 18s), and aggression updates (`ENEMY_AGGRESSION_INTERVAL` = 10s). The economy tick **banks the next miner upgrade as a reserve** — fighter upgrades, research, and fighter training may only spend coin above the reserve, otherwise the training drain keeps the wallet under 500/1500 forever and miners never advance past level 1. Miner training is exempt (miners pay for themselves) and the miner target grows with level (`4 + level * 2`). After the upgrade it buys fighter upgrades once a 400-coin cushion is safe (cheapest first), then research (sonar first, fortify when the base is hurt, then the fighter tech matching its most numerous fighter type); the sonar scan fires whenever its cooldown is up.
