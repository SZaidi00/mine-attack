...
A multi-phase revamp guide lives at `improvements/revamp.md` (with new sprites staged in `improvements/mine_attack_sprites/`). **Revamp Phase 1 (Fog of War & lanterns) is implemented**; later phases (factions, towers/walls, dynamic terrain, weather, tech-tree overhaul, AI belief system) are not.
...
- `constants.gd`
  ...
  - Fog of War (Revamp Phase 1): per-type `VISION_*` radii in cells (miner 6 surface / 4 underground, swordsman 8, archer 12, wizard 10, dragon 14, building 10), `FOG_MEMORY_DURATION` (10s), `FOG_COLOR` (`#05070a`), `FOG_MEMORY_ALPHA` (0.7), and the lantern tables (`LANTERN_T1/T2/T3_COST` 200/600/1000, vision 8/14/22, `LANTERN_HP` 500, max 3 surface, min 3 cells apart; `UNDERGROUND_LANTERN_*` 100g, vision 10, 200 HP, max 5, min 2 cells apart; `LANTERN_SALVAGE_RATIO` 0.5).
...
- `player_controller.gd`
  ...
  - Lantern placement mode (Revamp Phase 1): `start_build_placement("lantern" | "underground_lantern")` spawns a cursor ghost (green/red validity tint + vision-radius ring); left-click confirms via `try_place_lantern(kind, world_pos)` (validates, spends coin, spawns under `Main/Structures`, or upgrades an existing own surface lantern in place), right-click/Esc cancels. Fog-aware clicks: hidden enemy units and lanterns can't be clicked; right-clicking a visible enemy lantern orders an attack.
...
- `lantern.gd` (`class_name Lantern`, scene `scenes/lantern.tscn`, parented under `Main/Structures`)
  - Fog of War vision structure (Revamp Phase 1). Surface lanterns are tiered (T1 200g/8 cells → T2 600g/14 → T3 1000g/22, upgraded in place at the same cell, 500 HP, 5s build); underground lanterns are single-tier (100g, 10 cells, 200 HP, 3s build) and permanently reveal buried ore in their radius on completion (`reveal_ore_in_radius` — the same `sonar_revealed` mechanism as Ore Sonar).
...
- `unit.gd`
  ...
  - Defend leash (`_hold_post` / `_auto_engaged`): hold-style orders (`stop`, `garrison_home`) set `_hold_post`; movement/attack/rally orders clear it. Targets picked by the idle auto-attack scan are marked `_auto_engaged`. While both are set, the unit only notices targets within `UNIT_DEFEND_LEASH_RANGE` (400) of its post (never the enemy building — defend units never wander into a siege) and drops the chase once it strays past the leash, then walks home. Explicit orders are never leashed, so attack-mode and right-clicked units chase freely.
...
- `tests/test_defend_leash.gd` — defend leash: auto-engaged holders chase within 400px of the post and drop the target beyond it, held units ignore far targets and never auto-siege the enemy building, explicit attack/move orders are unleashed, garrison sets the hold.
