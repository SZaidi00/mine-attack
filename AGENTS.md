...
A multi-phase revamp guide lives at `improvements/revamp.md` (with new sprites staged in `improvements/mine_attack_sprites/`). **Revamp Phase 1 (Fog of War & lanterns) and Phase 2 (factions) are implemented**; later phases (towers/walls, dynamic terrain, weather, tech-tree overhaul, AI belief system) are not.
